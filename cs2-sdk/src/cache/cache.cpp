...
void CMatchCache::AddEntity(CEntityInstance* inst, CBaseHandle handle) {
    ...
    CEntityIdentity* identity = ent->m_pEntity();
    const char* name = identity ? identity->m_designerName() : nullptr;
    CLogger::Log("[cache] [+] {} -> {} | CHandle [Serial: {}, Index: {}]", name ? name : "unknown", SDK_LOG_PTR(ent),
